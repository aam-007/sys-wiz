...
from textual.widgets import Header, Footer, Static, Button, Tree, Label, Input, Log
...
#+#    #+#    #+#    #+#    #+#                #+#+# #+#+#      #+#      #+#      
 ########     ###     ########                  ###   ###   ########### #########   
        ...
        yield Container(
            ...
            Static(f"sys-wiz v0.1.0 | Author: Assistant", classes="meta"),
...
# --- 3. Command Preview Screen (Zero Magic) ---
...
class ExecutionScreen(Screen):
    ...
    def run_process(self):
        ...
        # Split command for subprocess, respecting quotes
    ...
    def finished(self, code: int):
        ...
        log.write("\nSuggested Action: Review output above. Click 'Back to Menu' to continue.")

    def on_button_pressed(self, event: Button.Pressed):
        if event.button.id == "back":
            # Pop execution, pop preview, back to menu
            self.app.pop_screen() 
            self.app.pop_screen()
...
class MainMenu(Screen):
    ...
    def on_tree_node_selected(self, event: Tree.NodeSelected):
        if not event.node.allow_expand: # It's a leaf
            cmd_def = event.node.data
            if cmd_def:
                self.initiate_command(cmd_def)

    def initiate_command(self, cmd_def: CommandDef):
        if cmd_def.needs_input:
            self.app.push_screen(
                InputRequestScreen(
                    cmd_def.input_prompt, 
                    lambda val: self.show_preview(cmd_def, val)
                )
            )
        else:
